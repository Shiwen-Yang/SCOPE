# Save this as interactive_loss_plot.py
...
def interactive_plot_with_landscape(loss_func, y, param_range=(-0.1, 0.1), param_label=r'$\rho$', N_param=30, M_theta=180):
    ...
    print("Precomputing...")
    loss_cache, hess1_cache, hess2_cache = compute_theta_loss_and_hessian_curves(loss_func, y1, y2, param_vals, thetas)
    ...
    h1_line, = ax2_right.plot(theta_vals_np, hess1_cache[0], linestyle='--', label="1st Hess Term")
    h2_line, = ax2_right.plot(theta_vals_np, hess2_cache[0], linestyle=':', label="2nd Hess Term")
    ...
    def update(val):
        ...
        h1_line.set_ydata(hess1_cache[pidx])
        h2_line.set_ydata(hess2_cache[pidx])
